...
@if ($indexAnggota < count($kelompok['anggota']) - 1) <hr class="m-0">
    ...
    <td>
        ...
        <div class="d-flex justify-content-between">
            <button class="btn btn-success w-50 m-1 btn-accept" data-id="{{ $kelompok['id'] }}" data-action="accept">
                <i class="fas fa-check"></i>
            </button>
            <button class="btn btn-danger w-50 m-1 btn-reject" data-id="{{ $kelompok['id'] }}" data-action="reject">
                <i class="fas fa-times"></i>
            </button>
        </div>
        @endif
                        ...
                        $.ajax({
                            ...
                            , success: function(response) {
                                ...
                                if (response.status === "exists") {
                                    ...
                                    $(`[data-id="${kelompokId}"][data-action="accept"]`)
                                ...
                                Swal.fire({
                                    ...
                                    if (actionType === "accept") {
                                        // Mengubah tombol "Terima" menjadi hanya ikon check dan disable
                                        $(`[data-id="${kelompokId}"][data-action="accept"]`)
                                            .html('<i class="fas fa-check"></i>') // Mengganti teks dengan ikon
                                            .prop("disabled", true); // Menonaktifkan tombol
                                    } else if (actionType === "reject") {
                                        $(`[data-id="${kelompokId}"][data-action="reject"]`)
                                            .html('<i class="fas fa-times"></i>') // Mengganti teks dengan ikon
                                            .prop("disabled", true);
                                        $(`[data-id="${kelompokId}"][data-action="accept"]`)
                                            .html('<i class="fas fa-check"></i>') // Mengganti teks dengan ikon
                                            .prop("disabled", false);

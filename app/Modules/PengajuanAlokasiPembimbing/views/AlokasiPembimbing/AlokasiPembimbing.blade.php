                        ...
                        <tr class="bg-light">
                            ...
                            <td class="align-middle status-cell" data-status="belum_fix">
                                <input type="text" class="form-control text-center pembimbing mb-2"
                                    data-index="{{ $index }}" name="pembimbing1{{ $row['nama_kota'] }}"
                                    list="dosenList"
                                    onchange="saveData('{{ $row['id_pengajuan_pembimbing'] }}', 'pembimbing1', this.value)">
                                <label>Status:</label>
                                <select class="form-control status-dropdown w-100" onchange="saveData('{{ $row['id_pengajuan_pembimbing'] }}', 'status_pembimbing1', this.value)">
                            ...
                            <td class="align-middle status-cell" data-status="belum_fix">
                                <input type="text" class="form-control text-center pembimbing mb-2"
                                    data-index="{{ $index }}" name="pembimbing2{{ $row['nama_kota'] }}"
                                    list="dosenList"
                                    onchange="saveData('{{ $row['id_pengajuan_pembimbing'] }}', 'pembimbing2', this.value)">
                                <label>Status:</label>
                                <select class="form-control status-dropdown w-100" onchange="saveData('{{ $row['id_pengajuan_pembimbing'] }}', 'status_pembimbing2', this.value)">
                                    <option value="belum_fix" selected>Belum Fix</option>
                                    <option value="fix">Fix</option>
                                </select>
                            </td>
                            ...
                            <td class="align-middle text-left">
                                <div class="detail-content">
                                    ...
                                    <div><strong>KoTA:</strong> P1:
                                        {{ $row['detailPembimbing']['pembimbing1_KoTA'] ?? '0' }} |
                                        P2: {{ $row['detailPembimbing']['pembimbing2_KoTA'] ?? '0' }} |
                                        Total: {{ $row['detailPembimbing']['jumlah_KoTA'] ?? '0' }}</div>
                                    <div><strong>Mhs:</strong> P1:
                                        {{ $row['detailPembimbing']['pembimbing1_Mhs'] ?? '0' }} |
                                        P2: {{ $row['detailPembimbing']['pembimbing2_Mhs'] ?? '0' }} |
                                        Total: {{ $row['detailPembimbing']['jumlahMahasiswa'] ?? '0' }}</div>
                                    <div><strong>Kuota:</strong> {{ $row['detailPembimbing']['kuota'] ?? '0' }}</div>
                                    <div><strong>Kelebihan:</strong>
                                        @if (($row['detailPembimbing']['jumlahMahasiswa'] ?? 0) > ($row['detailPembimbing']['kuota'] ?? 0))
                                            <span style="color: red;">
                                                {{ ($row['detailPembimbing']['jumlahMahasiswa'] ?? 0) - ($row['detailPembimbing']['kuota'] ?? 0) }}
                                                (Overload)
                                            </span>
                                        @else
                                            <span style="color: green;">Aman</span>
                            ...
                            <td class="align-middle">
                                <input type="text" class="form-control text-center penguji"
                                    data-index="{{ $index }}" name="penguji1{{ $row['nama_kota'] }}"
                                    list="dosenList"
                                    onchange="saveData('{{ $row['id_pengajuan_pembimbing'] }}', 'penguji1', this.value)">
                            </td>
                            <td class="align-middle">
                                <input type="text" class="form-control text-center penguji"
                                    data-index="{{ $index }}" name="penguji2{{ $row['nama_kota'] }}"
                                    list="dosenList"
                                    onchange="saveData('{{ $row['id_pengajuan_pembimbing'] }}', 'penguji2', this.value)">
                            </td>
                            <td class="align-middle">
                                <input type="text" class="form-control text-center penguji"
                                    data-index="{{ $index }}" name="penguji3{{ $row['nama_kota'] }}"
                                    list="dosenList"
                            ...
                            <td class="align-middle" style="min-width: 150px;">
                                <textarea class="form-control text-left auto-expand catatan-input"
                                    ...
                                    oninput="saveData('{{ $row['id_pengajuan_pembimbing'] }}', 'catatan', this.value)">
                                </textarea>
                            </td>
                        </tr>
                    @endforeach
...
@section('js')
    ...
    $(document).ready(function () {
        ...
        function saveData(id_pengajuan_pembimbing, key, value) {
            ...
            if (!found) {
                found = { id_pengajuan_pembimbing: id_pengajuan_pembimbing };

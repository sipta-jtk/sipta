...
@section('content')
    ...
    <div class="card-body">
        ...
        <x-adminlte-datatable id="dosenTable" :heads="$heads" striped hoverable  footer-theme="light" beautify :config="[
            'paging' => true,  
            'searching' => true,  
            'ordering' => true,  
        ...
        <div class="d-flex justify-content-end mb-2">
            ...
            @foreach ($dosen as $d)
                <tr>
                    ...
                    <td>    
                        <button class="btn btn-xs btn-default text-teal mx-1 shadow btn-update-dosen" title="Keys" data-toggle="modal" data-target="#updateDosen" class="bg-purple"
                        data-nip="{{ $d->nip }}"
                        data-nama="{{ $d->nama }}"
                        data-email="{{ $d->email }}"
                        ...
                        <i class="fa fa-lb fa-fw fa-pen"></i>
                </button>


                        @If ($d->role_dosen != 'kajur')
                            <button class="btn btn-xs btn-default text-danger mx-1 shadow btn-delete-dosen" title="Delete" data-toggle="modal" data-target="#deleteDosen" 
                                data-nip="{{$d->nip}}"
                                data-nama="{{$d->nama}}">
            ...
            @endforeach
                ...
                <x-adminlte-modal id="addNewDosen" title="Menambah Dosen Baru" theme="purple"
                 ...
                 <x-adminlte-modal id="deleteDosen" title="Menghapus Dosen" theme="purple"
                    icon="fa fa-trash" size='lg' disable-animations>
                ...
                <x-adminlte-modal id="updateDosen" title="Mengupdate Data Dosen" theme="purple"
                    ...
                    <form action="{{route('dosen.updateDosen')}}" method="POST">
                        @csrf
                        {{-- <x-adminlte-input name="old_nip"  id="old-nip" type="hidden" placeholder="NIP"
                                    fgroup-class="col-md-6" disable-feedback  required  /> --}}
                        <div class="row">
